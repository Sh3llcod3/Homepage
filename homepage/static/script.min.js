...
function getPreviousTrack(a){a=atob(a).replace("#","%23");var b=document.createElement("a");b.href=a;b.download=a.split("/").pop();document.body.appendChild(b);b.click();document.body.removeChild(b)}
...
qualityDict[document.getElementById("menu-opener-btn").innerHTML]);a.onreadystatechange=function(c){4==a.readyState&&200==a.status?(getPreviousTrack(c.target.responseText),window.location.reload(!1)):4==a.readyState&&300<=a.status&&(mdcSnackBar.listen("MDCSnackbar:closing",function(){mdcSnackBar.open()}),displayMDCSnackbar("Encountered an error, refresh the page and try again.","Refresh",function(){window.location.reload(!1)},1E4))};a.open("POST","./");a.send(b)}else displayMDCSnackbar("Please ensure that the link is valid and not empty.",
